...
test('n:asset with lazy loading', function () {
	$asset = new ImageAsset(
		url: 'https://example.com/image.jpg',
	...
	$result = $latte->renderToString('<img n:asset=foo>');
	Assert::same('<img src="https://example.com/image.jpg" width="176" height="104">', $result);
});
...
test('overwriting width / height', function () {
	...
	$result = $latte->renderToString('<img n:asset=foo width=1 alt="foo">');
	Assert::same('<img src="https://example.com/image.jpg" width=1 alt="foo">', $result);
});
