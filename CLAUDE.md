...
2. **Registry** - Central point managing multiple named mappers
   ...
   - Built-in FIFO cache (max 100 entries) for resolved assets — the oldest inserted entry is evicted via `array_shift()`; reading an entry does not refresh its position
   - Falls back to 'default' mapper when no prefix specified

3. **Asset** - Type-specific classes representing files
   - Base interface: `Asset` declares only `__toString()`; `$url` and `$file` are documented via `@property-read` (the real property declarations are commented out, waiting for PHP 8.4)
   - `HtmlRenderable` (extends `Asset`) adds `getImportElement()` and `getPreloadElement()` — only assets implementing it can be rendered as HTML
   - Specialized types: ImageAsset, ScriptAsset, StyleAsset, AudioAsset, VideoAsset, FontAsset, GenericAsset
   - All asset types implement `HtmlRenderable` **except `GenericAsset`** (which cannot be rendered as a tag, only resolved to a URL)
   - EntryAsset for Vite entry points with dependencies (extends `ScriptAsset`)
   - Some properties are lazy-loaded using the `LazyLoad` trait (see below)

### Asset Type Properties

Different asset types provide different readonly properties. Only the properties marked *(lazy)* are lazy-loaded via the `LazyLoad` trait; the rest are plain constructor parameters.

- **ImageAsset**: `width` *(lazy)*, `height` *(lazy)*, `mimeType` *(lazy)*, `alternative` (alt text), `lazyLoad` (bool → `loading="lazy"`), `density`, `crossorigin`
- **ScriptAsset**: `type`, `integrity`, `crossorigin`
- **StyleAsset**: `media`, `integrity`, `crossorigin`
- **AudioAsset**: `duration` *(lazy; estimated for MP3 via `Helpers::guessMP3Duration()`)*, `mimeType`
- **VideoAsset**: `width`, `height`, `mimeType`, `duration`, `poster`, `autoPlay` — none lazy-loaded; `duration` is a constructor parameter, not auto-detected
- **FontAsset**: `mimeType`, `integrity`, `crossorigin` (defaults to `true` for fonts, but can be overridden)
- **GenericAsset**: `mimeType` *(lazy; via `mime_content_type()`)*, `media`, `integrity` — does NOT implement `HtmlRenderable`
- **EntryAsset**: `imports` (`list<HtmlRenderable>`, rendered CSS dependencies), `preloads` (`list<HtmlRenderable>`, JS preloads), `type` (defaults to `'module'`), `integrity`, `crossorigin`

### Key Design Patterns

**Lazy Property Loading:**
- The `LazyLoad` trait defers expensive operations (reading dimensions, MIME types) until accessed
- Used only by `ImageAsset`, `AudioAsset`, and `GenericAsset` — the asset types that need file I/O for their properties. `ScriptAsset`, `StyleAsset`, `VideoAsset`, `FontAsset`, and `EntryAsset` do not use it (their properties are plain constructor parameters)
...
**Vite Integration:**
- Automatic dev server detection via `.vite/nette.json` (read by `Helpers::detectDevServer()`)
...
│   ├── Asset.php          # Base interface (declares __toString())
│   ├── HtmlRenderable.php # Interface for assets renderable as HTML elements
...
- Provides `{asset}` / `{asset?}` tags and `n:asset` / `n:asset?` attributes
...
- Actual rendering lives in `AssetsLatte/Runtime.php` (registered as the `assets` provider):
  - `renderAsset()` renders an asset's import element plus all `EntryAsset` dependencies (preloads + imports)
  - `renderAssetPreload()` renders the preload hint element
  - `renderAttributes()` outputs just the HTML attributes for `n:asset`, adapting `<img>`/`<script>` to `<link>` or `<a>` when the host tag differs, and completing `width`/`height` to preserve aspect ratio
  - All three throw if the asset does not implement `HtmlRenderable` (e.g. `GenericAsset`)

**Nonce / CSP support (`Runtime`):**
- `applyNonce()` automatically adds a `nonce` attribute to rendered `<script>`, `<link>`, and `<style>` elements
- The nonce value is extracted from the `Content-Security-Policy` (or `-Report-Only`) response header via `findNonce()`, matching `'nonce-...'`
- Resolved lazily and cached for the request; if no CSP nonce header is present, no nonce is added
...
- Registry cache is intentionally limited to 100 entries (FIFO eviction)
...
1. Vite dev server is running (auto-detected via `.vite/nette.json`)
